...
export function normalized(e: unknown): AcapError {
  return e instanceof AcapError ? e : new AcapError('INTERNAL');
}
...
export function descriptor(bytes: string | Uint8Array): Descriptor {
  ...
  for (const o of d.operations) {
    ...
    payloadSchema(o.output); // G3 supports the minimal unary/non-retrying transport only: reject unsupported promises before evaluation.
    check(
      o.kind === 'unary' && o.idempotency === 'none',
      'FAILED_PRECONDITION',
    );
  }
  return d;
