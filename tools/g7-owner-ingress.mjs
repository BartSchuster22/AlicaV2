...
export function createIngress(trustedConfig, identify = transportIdentity) {
  ...
  const handle = (socket) => {
    ...
    try {
      timer = setTimeout(close, Math.max(0, deadline - performance.now()));
    ...
    socket.on('end', () => {
      if (ended) return;
      try {
        if (expired() || identify(socket) !== pin || data.length < 4 || data.length !== data.readUInt32BE(0) + 4) return;
        ...
        if (expired()) return; // includes parsing/authentication; exclusive boundary
