...
Status: G7 implementation and scoped existing-host environment setup authorized. Both OS-level environment probes passed. Owner-approved online software keys and a checksum-verified off-server custody archive exist; operator trust-bootstrap tooling is implemented, but actual initial signing and independent public-Kernel verification are complete; acceptance-host pinning remains pending. [Concrete R1 contracts](CONTRACT-REVIEW-R1.md) now have executable model checks but await review/freeze. Packaging/Cell implementation and candidate qualification remain open. No G7 release is claimed.
...
| E03 | Trusted bootstrap and signing custody         | PARTIAL: owner-approved online custody on ElioHermes1, keys created and encrypted off-server copy verified; initial trust ceremony and public-Kernel verification complete; acceptance-host pins pending; see [custody](CUSTODY.md) | Owner holds passphrase; no unattended signer; bootstrap verifier delivery, root pinning and signed policy/revocation must still be verified; secrets never in chat/repo                           |
| E04 | Journal/restore contracts and limits          | CONCRETE R1 DRAFT TESTED; [review/freeze pending](CONTRACT-REVIEW-R1.md) per ADR-009                                                                                                                                                | Review closed schemas, atomicity/crash rules, archive/resource limits and encrypted backup format/dependencies before packaging code                                                              |
