...
Frames are four-byte unsigned big-endian byte length plus UTF-8 closed-schema JSON, <=65536 bytes, depth <=32, no zero/oversized/trailing frames or duplicate keys. One request/response per connection; cap at 8 connections and 30 s; mutation writer lock conflicts immediately. Errors return enumerated codes without private paths, tokens or secret values. A timeout is not proof of cancellation/rollback: inspect the durable operation outcome before retrying.
