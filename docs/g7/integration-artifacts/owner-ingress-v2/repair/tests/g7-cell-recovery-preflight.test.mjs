...
assert.deepEqual(h.dependencySpecifiers, ['./g7-owner-ingress.mjs']);
const productionIngress = await import('../tools/g7-owner-ingress.mjs');
const ingressDependency = new vm.SyntheticModule(['lookupOwnerCheckpointConfirmation'], function () {
  this.setExport('lookupOwnerCheckpointConfirmation', productionIngress.lookupOwnerCheckpointConfirmation);
}, { context });
await h.link(spec => spec === './g7-owner-ingress.mjs' ? ingressDependency : deny(spec));
await h.evaluate({ timeout: 1000 });
